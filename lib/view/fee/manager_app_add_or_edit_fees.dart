...
class ManagerAppAddOrEditFees extends StatefulWidget {
  static const route = '/manager-app-fees/add-fees';
  const ManagerAppAddOrEditFees({super.key});

  @override
  State<ManagerAppAddOrEditFees> createState() =>
      _ManagerAppAddOrEditFeesState();
}

class _ManagerAppAddOrEditFeesState extends State<ManagerAppAddOrEditFees> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      await feeCubit?.getBatchInvoiceForManager(
        batchInvoiceId: feeCubit?.student.id,
        managerId:
      ...
      body: BlocConsumer<FeeCubit, FeeState>(
        listener: (context, state) {
          if (state is AddedFees) {
            ...
            feeCubit?.getBatchInvoiceForManager(
              batchInvoiceId: feeCubit?.student.id,
              managerId: authCubit
                    ...
                    RichText(
                      ...
                      text: TextSpan(
                        children: [
                          ...
                          TextSpan(
                            text:
                                '₹${feeCubit?.batchFeeInvoice?.pendingAmount}',
                            style:
                                Theme.of(context).textTheme.bodyLarge?.copyWith(
                                      color: AppColors.primaryColor,
                                    ),
