...
class ManagerAppAddOrEditAdmissionFees extends StatefulWidget {
  static const route = '/fees/manager-app-add-admission-fees';
  const ManagerAppAddOrEditAdmissionFees({super.key});

  @override
  State<ManagerAppAddOrEditAdmissionFees> createState() =>
      _ManagerAppAddOrEditAdmissionFeesState();
}

class _ManagerAppAddOrEditAdmissionFeesState
    extends State<ManagerAppAddOrEditAdmissionFees> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      await feeCubit?.getAdmissionInvoiceForManager(
      ...
      body: BlocConsumer<FeeCubit, FeeState>(
        listener: (context, state) {
          if (state is AddedFees) {
            ...
            feeCubit?.getAdmissionInvoiceForManager(
                                  ...
                                  if (payment?.isDeleted == 1) {
                                    ...
                                    CommonDialog(
                                      ...
                                      onTap: () {
                                        ...
                                        feeCubit?.deleteAdmissionFeesForManager(
                                            managerId: authCubit
                                                    ?.user
                                    ...
                                    CommonDialog(
                                      ...
                                      onTap: () {
                                        ...
                                        if (formKey1.currentState!.validate()) {
                                          Navigator.pop(context);
                                          feeCubit?.editAdmissionFeesForManager(
                                              authCubit
                                                      ?.user
                                                      ?.managerDetail?[authCubit
                                                              ?.managerIndex ??
                          ...
                          child: Button(
                            onTap: () {
                              if (formKey.currentState!.validate()) {
                                feeCubit?.addFeesForAdmissionForManager(
                                    authCubit
                                            ?.user
                                            ?.managerDetail?[
