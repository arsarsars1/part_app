...
class TrainerAppAddOrEditAdmissionFees extends StatefulWidget {
  static const route = '/fees/trainer-app-add-admission-fees';
  const TrainerAppAddOrEditAdmissionFees({super.key});

  @override
  State<TrainerAppAddOrEditAdmissionFees> createState() =>
      _AddOrEditFeesState();
}

class _AddOrEditFeesState extends State<TrainerAppAddOrEditAdmissionFees> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      await feeCubit?.getAdmissionInvoiceForTrainer(
          authCubit?.user?.trainerDetail?[authCubit?.trainerIndex ?? 0].id ?? 0,
          feeCubit?.student.id);
    });
    super.initState();
  }
      ...
      body: BlocConsumer<FeeCubit, FeeState>(
        listener: (context, state) {
          if (state is AddedFees) {
            ...
            feeCubit?.getAdmissionInvoiceForTrainer(
                                  ...
                                  if (payment?.isDeleted == 1) {
                                    ...
                                    CommonDialog(
                                      ...
                                      onTap: () {
                                        ...
                                        feeCubit?.deleteAdmissionFeesForTrainer(
                                            trainerId: authCubit
                                                    ?.user
                                    ...
                                    CommonDialog(
                                      ...
                                      onTap: () {
                                        ...
                                        if (formKey1.currentState!.validate()) {
                                          Navigator.pop(context);
                                          feeCubit?.editAdmissionFeesForTrainer(
                                              authCubit
                                                      ?.user
                                                      ?.trainerDetail?[authCubit
                                                              ?.trainerIndex ??
                          ...
                          child: Button(
                            onTap: () {
                              if (formKey.currentState!.validate()) {
                                feeCubit?.addFeesForAdmissionForTrainer(
                                    authCubit
                                            ?.user
                                            ?.trainerDetail?[
