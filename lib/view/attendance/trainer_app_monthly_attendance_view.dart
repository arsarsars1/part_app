...
class TrainerAppMonthlyAttendanceView extends StatefulWidget {
  static const route = '/trainer-app-month_or_year_attendance';

  const TrainerAppMonthlyAttendanceView({Key? key}) : super(key: key);

  @override
  State<TrainerAppMonthlyAttendanceView> createState() =>
      _TrainerAppMonthlyAttendanceViewState();
}

class _TrainerAppMonthlyAttendanceViewState
    extends State<TrainerAppMonthlyAttendanceView> {
  ...
  void doSearch(bool clean) {
    context.read<AttendanceCubit>().getStudentsMonthlyForTrainer(
          trainerId: authCubit
                  ?.user?.trainerDetail?[authCubit?.trainerIndex ?? 0].id ??
              0,
          batchId: batch?.id,
