...
class TrainerAppAttendanceCalenderView extends StatefulWidget {
  ...
  State<TrainerAppAttendanceCalenderView> createState() =>
      _TrainerAppAttendanceCalenderViewState();
}

class _TrainerAppAttendanceCalenderViewState
    extends State<TrainerAppAttendanceCalenderView> {
  bool isActive = true;
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      await cubit.getConductedClassesOfMonthForTrainer(
          ...
          batchId: cubit.id,
          date: DateTime(currentYear, currentMonth));
      await studentCubit?.getStudentsForTrainer(
          trainerId: authCubit
                  ?.user?.trainerDetail?[authCubit?.trainerIndex ?? 0].id ??
              0,
          batchId: cubit.id,
                              ...
                              if ((studentCubit?.students?.length ?? 0) == 0) {
                                ...
                                if (DateTime.now().compareTo(
                                  ...
                                  if (events.length > 1) {
                                    ...
                                    await cubit
                                        .getConductedClassesOfMonthForTrainer(
                                            trainerId: authCubit
                                                    ?.user
