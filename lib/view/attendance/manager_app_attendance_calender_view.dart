...
import 'package:part_app/view/attendance/components/manager_app_attendance_class_pick.dart';
...
class ManagerAppAttendanceCalenderView extends StatefulWidget {
  static const route = '/manager-app-batch-calender-view';

  const ManagerAppAttendanceCalenderView({super.key});

  @override
  State<ManagerAppAttendanceCalenderView> createState() =>
      _ManagerAppAttendanceCalenderViewState();
}

class _ManagerAppAttendanceCalenderViewState
    extends State<ManagerAppAttendanceCalenderView> {
  bool isActive = true;
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      await cubit.getClassesOfMonthForManager(
          managerId: authCubit
                  ?.user?.managerDetail?[authCubit?.managerIndex ?? 0].id ??
              0,
          batchId: cubit.id,
          date: DateTime(currentYear, currentMonth));
      await cubit.getConductedClassesOfMonthForManager(
          managerId: authCubit
                  ?.user?.managerDetail?[authCubit?.managerIndex ?? 0].id ??
              0,
          batchId: cubit.id,
          date: DateTime(currentYear, currentMonth));
      await studentCubit?.getStudentsForManager(
          managerId: authCubit
                  ?.user?.managerDetail?[authCubit?.managerIndex ?? 0].id ??
              0,
          batchId: cubit.id,
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        context.read<AttendanceCubit>().getBatchesByStatusForManager(
              managerId: authCubit
                      ?.user?.managerDetail?[authCubit?.managerIndex ?? 0].id ??
                  0,
                        ...
                        CalendarCarousel<EventModel>(
                                ...
                                if (DateTime.now().compareTo(
                                  ...
                                  if (events.length > 1) {
                                    ...
                                    scaffoldKey.currentState?.showBottomSheet(
                                        ...
                                        (context) =>
                                            ManagerAppAttendanceClassPicker(
                                              date: date,
                                              eventList: events,
                                            ));
                                    await cubit.getClassesOfMonthForManager(
                                    ...
                                    await cubit
                                        .getConductedClassesOfMonthForManager(
                                            managerId: authCubit
                                                    ?.user
                                    ...
                                    if (cubit.conductedClassIdList[0] == 0) {
                                      ...
                                      Navigator.pushNamed(
                                        context,
                                        ManagerAppAttendanceUpdate.route,
                                      );
                                    }
                                  }
                                }
                          ...
                          onLeftArrowPressed: () async {
                            ...
                            await cubit.getClassesOfMonthForTrainer(
                                ...
                                batchId: cubit.id,
                                date: DateTime(currentYear, currentMonth));
                            await cubit.getConductedClassesOfMonthForTrainer(
                                trainerId: authCubit
                                        ?.user
                                        ?.trainerDetail?[
                                            authCubit?.trainerIndex ?? 0]
