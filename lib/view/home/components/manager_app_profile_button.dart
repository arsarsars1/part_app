...
class ManagerProfileButton extends StatefulWidget {
  const ManagerProfileButton({super.key});

  @override
  State<ManagerProfileButton> createState() => _ManagerProfileButtonState();
}

class _ManagerProfileButtonState extends State<ManagerProfileButton> {
  @override
        ...
        child: BlocBuilder<ProfileCubit, ProfileState>(
          builder: (context, state) {
            ...
            if (state is ProfileInitial) {
              url = user?.managerDetail?[authCubit.managerIndex].profilePic !=
                      ""
                  ? '${F.baseUrl}/managers/${authCubit.user?.managerDetail?[authCubit.managerIndex].id}/images/profile-pic/${authCubit.user?.managerDetail?.first.profilePic}'
                  : user?.managerDetail?[authCubit.managerIndex].gender ==
                          "male"
                      ? "https://v1.partapp.in/images/avatars/avatar-5.png"
                      : "https://v1.partapp.in/images/avatars/avatar-1.png";
            } else if (state is ProfileLoaded) {
              url =
                  '${F.baseUrl}/managers/${authCubit.user?.managerDetail?[authCubit.managerIndex].id}/images/profile-pic/${state.profilePic}';
            }
