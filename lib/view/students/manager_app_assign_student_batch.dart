...
class ManagerAppAssignStudentBatch extends StatefulWidget {
  static const route = '/manager-app-students/add/assign-batch';

  final bool editStudent;

  const ManagerAppAssignStudentBatch({super.key, required this.editStudent});

  @override
  State<ManagerAppAssignStudentBatch> createState() =>
      _ManagerAppAssignStudentBatchState();
}

class _ManagerAppAssignStudentBatchState
    extends State<ManagerAppAssignStudentBatch> {
        ...
        listener: (context, state) {
          if (state is CreatingStudent) {
            ...
            if (widget.editStudent) {
              ...
              context.read<StudentCubit>().getStudentBatchesForManager(
              ...
              cubit.studentDetailsForManager(
                  managerId: authCubit?.user
                          ?.managerDetail?[authCubit?.managerIndex ?? 0].id ??
                      0,
                  studentId: cubit.tempStudent?.studentDetail?[0].id);
